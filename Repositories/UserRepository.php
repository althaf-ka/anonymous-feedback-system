...
class UserRepository
{
    ...
    public function getFulfilledSuggestions(int $limit = 5): array
    {
        return $this->db->fetchAll(
            ...
            COUNT(v.id) AS votes,
            CASE
                WHEN f.resolved_at IS NULL THEN 'Just now'
                
                WHEN TIMESTAMPDIFF(MINUTE, f.resolved_at, NOW()) < 60 
                THEN CONCAT(TIMESTAMPDIFF(MINUTE, f.resolved_at, NOW()), ' minutes ago')
            
                WHEN TIMESTAMPDIFF(HOUR, f.resolved_at, NOW()) < 24
                THEN CONCAT(TIMESTAMPDIFF(HOUR, f.resolved_at, NOW()), ' hours ago')
            
                ELSE CONCAT(TIMESTAMPDIFF(DAY, f.resolved_at, NOW()), ' days ago')
            END AS resolved_at 
        FROM feedbacks f
        JOIN categories c ON f.category_id = c.id
        LEFT JOIN feedback_votes v ON v.feedback_id = f.id -- ADDED: This line joins the votes table
        WHERE f.is_public = 1 AND f.allow_public = 1 AND f.status = 'resolved'
        GROUP BY f.id
